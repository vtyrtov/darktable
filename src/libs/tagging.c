...
static void _pop_menu_dictionary_delete_tag(GtkWidget *menuitem, dt_lib_module_t *self, gboolean branch)
{
  ...
  if(img_count > 0 || dt_conf_get_bool("plugins/lighttable/tagging/ask_before_delete_tag"))
  {
    ...
    text = g_markup_printf_escaped(ngettext("do you really want to delete the tag `%s'?\n<u>%d</u> image is assigned this tag!",
             "do you really want to delete the tag `%s'?\n<u>%d</u> images are assigned this tag!", img_count), tagname, img_count);
...
static void _pop_menu_dictionary_create_tag(GtkWidget *menuitem, dt_lib_module_t *self)
{
  ...
  dt_osx_disallow_fullscreen(dialog);
#endif
  dt_gui_dialog_add(GTK_DIALOG(dialog), dt_gui_hbox(gtk_label_new(_("name: ")), dt_gui_expand(entry)),
...
static void _lib_tagging_tag_show(dt_action_t *action)
{
  ...
  // Don't set transient parent here on Wayland - will be set at GdkWindow level after realize
  // to ensure we use the correct toplevel window
#ifdef GDK_WINDOWING_WAYLAND
  ...
  gtk_widget_realize(d->floating_tag_window);

  // Position the window. On Wayland we must use gdk_window_move_to_rect() for proper positioning
  // as gtk_window_move() doesn't work reliably there.
#ifdef GDK_WINDOWING_WAYLAND
  if(GDK_IS_WAYLAND_DISPLAY(gtk_widget_get_display(d->floating_tag_window)))
  {
    // Get windows for positioning
    GdkWindow *center_window = gtk_widget_get_window(center);

    // Get the actual top-level window - this is critical for Wayland
    GdkWindow *toplevel_window = gdk_window_get_toplevel(gtk_widget_get_window(window));

    // Get positions to calculate center widget location relative to toplevel window
    gint toplevel_x, toplevel_y, center_x, center_y;
    gdk_window_get_origin(toplevel_window, &toplevel_x, &toplevel_y);
    gdk_window_get_origin(center_window, &center_x, &center_y);

    const gint w = gdk_window_get_width(center_window);
    const gint h = gdk_window_get_height(center_window);

    // Create a rectangle representing the anchor point at bottom-center of the center widget
    // Coordinates must be relative to the toplevel window (transient parent)
    GdkRectangle rect;
    rect.x = (center_x - toplevel_x) + w / 2 - FLOATING_ENTRY_WIDTH / 2;
    rect.y = (center_y - toplevel_y) + h - 50;
    rect.width = FLOATING_ENTRY_WIDTH;
    rect.height = 1;

    // Get the popup's GdkWindow and set its transient parent
    GdkWindow *popup_window = gtk_widget_get_window(d->floating_tag_window);
    gdk_window_set_transient_for(popup_window, toplevel_window);

    // Use move_to_rect for Wayland-compatible positioning
...
#endif
  {
    // Fallback to gtk_window_move() for X11 and other platforms
    gint px, py, w, h;
